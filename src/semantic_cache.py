...
class SemanticCache:
    ...
    def find_similar_query(self, query: str) -> Optional[Tuple[str, float]]:
        ...
        # Extract exercise number from query if present
        query_exercise = re.search(r'exercise\s*(\d+\.?\d*)', query.lower())
        
        best_similarity = 0.0
        best_query = None
        
        for cached_query in self.cache.keys():
            # Extract exercise number from cached query
            cached_exercise = re.search(r'exercise\s*(\d+\.?\d*)', cached_query.lower())
            
            # If both queries mention exercises but they're different, skip this cached query
            if query_exercise and cached_exercise and query_exercise.group(1) != cached_exercise.group(1):
                continue
